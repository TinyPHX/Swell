...
namespace Swell
{
    ...
    public class SwellMesh : MonoBehaviour
    {
        ...
        {
            public Dictionary<(float, float), int> positionToIndex = new Dictionary<(float, float), int>();
            public Dictionary<float, List<float>> xByY = new Dictionary<float, List<float>> ();
            public Dictionary<float, List<float>> yByX = new Dictionary<float, List<float>> ();
            ...
            {
                if (!positionToIndex.ContainsKey((x, y)))
                {
                    positionToIndex.Add((x, y), index);
        ...
        public void GenerateMesh()
        {
            ...
            if ((!meshInitialized || valueChanged) && valid)
            {
                ...
                if (mesh.subMeshCount > 0)
                {
                    ...
                    {
                        MeshLevel topLevel = Levels[Levels.Length - 1];
        ...
        private void UpdateMeshLowPoly(Mesh mesh)
        {
            if (LowPolyNormals)
            {
                if (mesh.vertices.Length != mesh.triangles.Length)
                {
                    ...
                    {
                        mesh.SetTriangles(tris2.Take(tris2.Length / 2).ToList(), 0);
                        mesh.SetTriangles(tris2.Skip(tris2.Length / 2).ToList(), 1);
                    }
                }
            }
        }
            ...
            for (float xi = 0; xi <= meshSize - step; xi += step)
            {
                for (float yi = 0; yi <= meshSize - step; yi += step)
                {
                    ...
                    if (x < -innerDistance || y < -innerDistance || x >= innerDistance || y >= innerDistance)
                    {
                        ...
                        pointsInSquare[0] = new Vector2(x, y);
                        pointsInSquare[1] = new Vector2(x, y + step);
                        pointsInSquare[2] = new Vector2(x + step, y + step);
                        pointsInSquare[3] = new Vector2(x + step, y);
