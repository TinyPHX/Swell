...
public class SwellWave : MonoBehaviour
{
    ...
    public float GetFluctuateRatio()
    {
        ...
        if (fluctuate)
        {
            fluctuateRatio = fluctuateCurve.Evaluate((Time.time / fluctuatePeriodTime + fluctuateOffset) % fluctuateCurve.keys[fluctuateCurve.length-1].time);
        }
        else
        {
            fluctuateRatio = 1;
    ...
    public float GetHeight(float x, float y)
    {
        ...
        if (waveRotation != 0)
        {
            ...
            Vector3 rotatedPosition = Quaternion.AngleAxis(-waveRotation, Vector3.up) * new Vector3(position.x - x, 0, position.z - y);
